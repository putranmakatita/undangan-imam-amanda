...
export default function Wishes() {
    ...
    /**
     * Function to fetch wishes from Google Sheet (Read operation - GET)
     */
    const fetchWishes = async () => {
        ...
        try {
            ...
            // Filter out incomplete wishes and set state
            setWishes(data.filter(w => w.name && w.message));
        } catch (err) {
            console.error("Failed to fetch wishes:", err);
            setError("Gagal memuat pesan. Silakan cek koneksi internet Anda atau coba lagi nanti.");
        } finally {
            setIsLoading(false);
        }
    };

    // Fetch wishes when the component mounts
    useEffect(() => {
        fetchWishes();
    }, []);

    /**
     * Function to submit a new wish to Google Sheet (Write operation - POST)
     */
    const handleSubmitWish = async (e) => {
        e.preventDefault();
        if (!name.trim() || !newWish.trim() || !attendance) {
            alert('Harap isi Nama, Kehadiran, dan Pesan Anda.');
            return;
        }
        ...
        const payload = {
            name: name,
            message: newWish,
            // Format for compatibility with the Apps Script logic
        ...
        try {
            ...
            if (result.success && result.wish) {
                // Add the new wish (received from the Apps Script response) to the top of the local state
    ...
    const getAttendanceLabel = (value) => {
        ...
        return option ? option.label : 'Pilih kehadiran...';
    };
    ...
    return (<>
        <section id="wishes" className="min-h-screen relative overflow-hidden">
            ...
            <div className="container mx-auto px-4 py-20 relative z-10">
                ...
                {/* Loading/Error State */}
                {isLoading && (
                    <div className="text-center py-12 text-gray-500 flex justify-center items-center space-x-2">
                ...
                {error && (
                    <div className="text-center py-4 text-rose-600 bg-rose-50 border border-rose-200 rounded-xl max-w-2xl mx-auto">
                        {error}
                    </div>
                )}
                                            ...
                                            <div className="flex items-start space-x-3 mb-2">
                                                ...
                                                <div className="flex-shrink-0">
                                                    <div className="w-8 h-8 rounded-full bg-gradient-to-r from-rose-400 to-pink-400 flex items-center justify-center text-white text-sm font-medium">
                                                        {wish.name ? wish.name[0].toUpperCase() : 'G'}
                                                    </div>
                                                </div>
